...
```{r}
# app.R
# QTL Design Sandbox (Shiny, QC + Missing Handling + Model Button + Safe Power Tab)
# -----------------------------------------------------------------------------
...
simulate_trait_gaussian <- function(G, qtl_index, beta, h2 = 0.2, rc2 = 0, seed = NULL) {
  ...
  var_g <- var(g_eff); if (var_g < 1e-12) var_g <- 1e-12
  sigma2_e <- (var_g - h2 * var_g) / h2
...
# ----- PATCH #2: robust simulation (clamp n to [2, N], guard M) -----
power_simulate <- function(G, qtl_index, beta, n, B = 200, alpha = 0.05,
                           alpha_method = "bonferroni", seed = 1) {
  set.seed(seed)
  M <- ncol(G)
  if (M < 1) return(0)                 # no markers → 0 power
  qtl_index <- max(1, min(qtl_index, M))
  ...
  tabPanel("Model",
    sidebarLayout(
      sidebarPanel(
        ...
        numericInput("beta_eff_model", "QTL effect β (standardised)", 0.2, step = 0.05),
        actionButton("btn_model_fit", "Run model analysis")
      ),
      mainPanel(
        h4("Model analysis summary"),
        verbatimTextOutput("model_summary"),
        h4("t-distribution (df = n_eff − 2) with observed t"),
...
server <- function(input, output, session) {
  ...
  # ----- PATCH #1: robust active matrix (fallback to demo when empty) -----
  G_active <- reactive({
    # prefer filtered → missing-handled → raw
    ...
    # fall back to demo when NULL / too few rows / no columns
  ...
  # ---------------- Model page (safe for realistic demo with NA) ----------------
  model_fit_res <- eventReactive(input$btn_model_fit, {
    G <- G_active()
    qidx <- as.integer(input$qtl_idx_model); if (qidx < 1 || qidx > ncol(G)) qidx <- 1
  ...
  output$t_plot <- renderPlot({
    ...
    ggplot(data.frame(x=xs, y=dens), aes(x,y)) +
      ...
      theme_minimal() +
      labs(title = glue("t-distribution (df={df}); dashed=±t_crit, solid=t_obs"),
           x = "t", y = "density")
  })
  ...
  power_df <- eventReactive(input$btn_run, {
    ...
    # ----- PATCH #3: robust qidx, grid n, and maf -----
    qidx <- as.integer(input$qtl_idx)
    M <- ncol(G)
    qidx <- if (is.finite(qidx) && qidx >= 1 && qidx <= M) qidx else 1
    ...
    # clamp n to [2, N]; if empty, use a safe single value
    N <- nrow(G)
    n_range <- n_range[n_range >= 2 & n_range <= N]
    if (length(n_range) == 0) n_range <- pmin(max(2, floor(N/2)), N)

    alpha_eff <- effective_alpha(alpha, M, method)

    # safe maf (fallback to 0.25)
    if (M >= 1) {
  ...
  output$stop_msg <- renderText({
    ...
    else glue("Suggested stopping point: n ≈ {df$n[min(below)]} (dPower/dn ≤ {input$deriv_thresh})")
  })
  ...
  solve_res <- eventReactive(input$btn_solve, {
    ...
    solve_n_for_power(G, qtl_index = qidx, beta = input$beta_eff,
...
```{r}
# app.R
# QTL Design Sandbox (Shiny, robust Power tab + safe indices/grid)
# ----------------------------------------------------------------
suppressPackageStartupMessages({
  library(shiny); library(shinythemes)
  library(data.table); library(DT); library(ggplot2)
...
# 取样本量网格的安全区间；当 grid/step 含 NA 时给出兜底
...
power_simulate <- function(G, qtl_index, beta, n, B = 200, alpha = 0.05,
  ...
  M <- ncol(G)
  if (M < 1) return(0)                 # no markers → 0 power
  qtl_index <- max(1L, min(as.integer(qtl_index), M))
  alpha_eff <- effective_alpha(alpha, M, alpha_method)
  out <- logical(B)
  N <- nrow(G)
  n_eff <- max(2L, min(as.integer(n), N))  # clamp to [2, N]
...
server <- function(input, output, session) {
  ...
  # ---------------- Power page (robust with safe helpers) ----------------
  power_df <- eventReactive(input$btn_run, {
    G <- G_active(); req(G)
...
```{r}
# app.R
# QTL Design Sandbox (Shiny, robust Power tab + safe indices/grid + NA-safe simulation)
# ----------------------------------------------------------------
suppressPackageStartupMessages({
  library(shiny); library(shinythemes)
  library(data.table); library(DT); library(ggplot2)
...
# Trait + association
# -----------------------------
# PATCH 1: NA-safe simulate_trait_gaussian (impute QTL column; guard variances)
simulate_trait_gaussian <- function(G, qtl_index, beta, h2 = 0.2, rc2 = 0, seed = NULL) {
  if (!is.null(seed)) set.seed(seed)
  n <- nrow(G)
...
# -----------------------------
# PATCH 2: Robust simulation (clamp n; column-mean imputation for Gb)
# -----------------------------
power_simulate <- function(G, qtl_index, beta, n, B = 200, alpha = 0.05,
                           alpha_method = "bonferroni", seed = 1) {
  set.seed(seed)
  M <- ncol(G)
  if (!is.finite(M) || M < 1L) return(0)  # no markers → 0 power
...
server <- function(input, output, session) {
  ...
  solve_res <- eventReactive(input$btn_solve, {
    tryCatch({
      ...
      showNotification(paste("Solver failed:", conditionMessage(e)),
                       type = "error", duration = 8)
...
```{r}
# app.R
# QTL Design Sandbox (Shiny, robust Power tab + safe indices/grid + NA-safe simulation + fast-sim toggle)
# ----------------------------------------------------------------
suppressPackageStartupMessages({
  library(shiny); library(shinythemes)
  library(data.table); library(DT); library(ggplot2)
